...
export function ReferenceForm({ reference, onClose }: { reference: Partial<Ref> | null; onClose: (s: boolean) => void }) {
  ...
  const inp = (label: string, key: keyof Ref, req = false) => (
    <div>
      <label className="block text-sm font-medium text-gray-600 mb-1.5">{label}{req && <span className="text-[#C8A96E] ml-1">*</span>}</label>
      <input
        type="text" value={(form[key] as string) ?? ''}
        onChange={(e) => set(key, e.target.value)}
        className="w-full px-3 py-2.5 bg-white border border-gray-300 rounded-xl text-gray-900 text-sm focus:outline-none focus:border-[#C8A96E] transition-colors"
      />
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-xl max-h-[90vh] overflow-y-auto bg-white border border-gray-200 rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-lg font-bold text-gray-900">{form.id ? 'Modifier la référence' : 'Nouvelle référence'}</h2>
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Sectors */}
          <div>
            <label className="block text-sm font-medium text-gray-600 mb-2">Secteurs d'activité</label>
                ...
                <button
                  ...
                  className={`text-xs px-3 py-1.5 rounded-full border transition-all ${
                    form.sectors.includes(s)
                      ...
                      : 'border-gray-300 text-gray-500 hover:border-slate-400'
                  }`}
                >
                  {s}
                </button>
          ...
          {/* Description */}
          <div>
            <label className="block text-sm font-medium text-gray-600 mb-1.5">Description courte</label>
            <textarea
          ...
          <div>
            <label className="block text-sm font-medium text-gray-600 mb-1.5">Description du projet</label>
            <textarea
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="flex items-center gap-2 col-span-2">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-gray-600">Mise en avant</span>
              </label>
