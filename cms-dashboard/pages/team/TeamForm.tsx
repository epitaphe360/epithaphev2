    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-xl max-h-[90vh] overflow-y-auto bg-[#0B1121] border border-[#1E293B] rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-[#1E293B]">
          <h2 className="text-lg font-bold text-white">{form.id ? 'Modifier le membre' : 'Nouveau membre'}</h2>
          <button onClick={() => onClose(false)}><X className="w-5 h-5 text-slate-400 hover:text-white" /></button>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1.5">Nom <span className="text-[#C8A96E]">*</span></label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1.5">Poste / Titre</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1.5">Département</label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">URL Photo</label>
            ...
            {form.socialLinks.map((sl, i) => (
              <div key={i} className="flex gap-2 mb-2">
                <select value={sl.platform} onChange={(e) => setSocial(i, 'platform', e.target.value)}
                  className="w-32 px-2 py-2 bg-[#0B1121] border border-[#334155] rounded-xl text-white text-sm focus:outline-none focus:border-[#C8A96E]">
                ...
                <button type="button" onClick={() => removeSocial(i)} className="p-2 text-slate-500 hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2">
              <label className="text-sm text-slate-300">Ordre</label>
              <input type="number" value={form.order} onChange={(e) => set('order', parseInt(e.target.value) || 0)}
                className="w-20 px-3 py-2 bg-[#0B1121] border border-[#334155] rounded-xl text-white text-sm focus:outline-none focus:border-[#C8A96E]" />
            </div>
          </div>
        </div>
