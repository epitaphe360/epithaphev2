...
import {
  ...
  BookOpen, BarChart2, Calculator, Mail, LogIn, ArrowRight,
} from "lucide-react";
...
const navConfig: NavEntry[] = [
  {
    ...
    mega: true,
    entries: [
      { label: "Conventions & Kickoffs",   description: "Fédérez vos équipes autour de vos ambitions",   href: "/evenements/conventions-kickoffs", icon: <Calendar className="w-5 h-5" /> },
      { label: "Soirées de gala",          description: "Créez des moments d'exception inoubliables",    href: "/evenements/soirees-de-gala",      icon: <Trophy   className="w-5 h-5" /> },
      { label: "Roadshows & Tournées",     description: "Portez votre message dans toute la région",    href: "/evenements/roadshows",            icon: <Truck    className="w-5 h-5" /> },
      { label: "Salons & Expositions",     description: "Maximisez votre visibilité sur les salons B2B",href: "/evenements/salons",               icon: <Store    className="w-5 h-5" /> },
    ],
  },
  {
    ...
    mega: true,
    entries: [
      { label: "Marque Employeur",    description: "Attirez et retenez les meilleurs talents",    href: "/architecture-de-marque/marque-employeur",   icon: <Briefcase   className="w-5 h-5" /> },
      { label: "Communication QHSE", description: "Sécurité & conformité avec impact visuel",    href: "/architecture-de-marque/communication-qhse", icon: <ShieldCheck className="w-5 h-5" /> },
      { label: "Expérience Clients", description: "Différenciation et fidélisation durables",    href: "/architecture-de-marque/experience-clients", icon: <Users       className="w-5 h-5" /> },
    ],
  },
  {
    label: "La Fabrique",
    mega: true,
    entries: [
      { label: "Impression grand format", description: "Bâches, adhésifs, toiles rétroéclairées",    href: "/la-fabrique/impression",   icon: <Printer   className="w-5 h-5" /> },
      { label: "Menuiserie & Décor",      description: "Stands sur mesure, mobilier d'ambiance",     href: "/la-fabrique/menuiserie",   icon: <Hammer    className="w-5 h-5" /> },
      { label: "Signalétique",            description: "Totems, enseignes, wayfinding professionnel",href: "/la-fabrique/signaletique", icon: <Signpost  className="w-5 h-5" /> },
      { label: "Aménagement Espace",      description: "Scénographie & architecture éphémère",       href: "/la-fabrique/amenagement",  icon: <LayoutGrid className="w-5 h-5" /> },
    ],
  },
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={`absolute top-full left-0 mt-2 bg-card border border-border rounded-2xl shadow-xl z-[100] overflow-hidden ${entry.mega ? "w-[640px] -left-20" : "w-72"}`}
          >
            ...
            {entry.mega && (
              <div className="px-4 py-3 bg-muted/50 border-t border-border flex items-center justify-between">
                ...
                <span className="flex items-center gap-1 text-xs font-semibold text-primary">Voir tout <ArrowRight className="w-3 h-3" /></span>
              </div>
            )}
          </motion.div>
        )}
