...
function fieldClass() {
  return 'block w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500';
}
  ...
  return (
    <div className="space-y-4">
      ...
      <CollapsibleSection title="🎯 Hero (bandeau principal)">
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="block text-xs font-medium text-gray-700 mb-1">Titre principal *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Tag (badge hero)</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Bouton CTA — Lien</label>
      ...
      <CollapsibleSection title="📢 Pitch (accroche argumentaire)">
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">Titre de l'accroche *</label>
          <input className={fieldClass()} value={value.pitchTitle} onChange={e => set('pitchTitle', e.target.value)} />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">Corps du texte *</label>
          <textarea className={fieldClass()} rows={3} value={value.pitchBody} onChange={e => set('pitchBody', e.target.value)} />
        </div>

        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-xs font-medium text-gray-700">Statistiques (chiffres clés)</label>
      ...
      <CollapsibleSection title="🔧 Blocs de services">
          ...
          {(value.serviceBlocks ?? []).map((block, i) => (
            <div key={i} className="border border-gray-200 rounded-lg p-4 space-y-3 bg-gray-50">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-gray-600 uppercase">Bloc {i + 1}</span>
                <button type="button" onClick={() => removeBlock(i)} className="text-red-400 hover:text-red-600">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-medium text-gray-700">Points clés (features)</label>
                ...
                <div className="space-y-1">
                  {(block.features ?? []).map((feat, fi) => (
                    <div key={fi} className="flex gap-2">
                      ...
                      <button type="button" onClick={() => removeFeature(i, fi)} className="text-red-400 hover:text-red-600 p-1">
        ...
        <button type="button" onClick={addBlock} className="w-full py-2 border-2 border-dashed border-gray-300 rounded-lg text-sm text-gray-500 hover:border-blue-400 hover:text-blue-600 flex items-center justify-center gap-2">
        ...
        <div className="space-y-2">
          {(value.references ?? []).map((ref, i) => (
            <div key={i} className="flex gap-2">
              ...
              <button type="button" onClick={() => removeRef(i)} className="text-red-400 hover:text-red-600 p-1">
                <Trash2 className="w-4 h-4" />
              </button>
            </div>
          ))}
        </div>
        <button type="button" onClick={addRef} className="w-full py-2 border-2 border-dashed border-gray-300 rounded-lg text-sm text-gray-500 hover:border-blue-400 hover:text-blue-600 flex items-center justify-center gap-2">
      ...
      <CollapsibleSection title="💬 Témoignages" defaultOpen={false}>
        <div className="space-y-4">
          {(value.testimonials ?? []).map((t, i) => (
            <div key={i} className="border border-gray-200 rounded-lg p-4 space-y-3 bg-gray-50">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-gray-600">Témoignage {i + 1}</span>
                <button type="button" onClick={() => removeTestimonial(i)} className="text-red-400 hover:text-red-600">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Note (1–5)</label>
        ...
        <button type="button" onClick={addTestimonial} className="w-full py-2 border-2 border-dashed border-gray-300 rounded-lg text-sm text-gray-500 hover:border-blue-400 hover:text-blue-600 flex items-center justify-center gap-2">
      ...
      <CollapsibleSection title="🚀 Call-to-action final">
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">Titre CTA *</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">Texte CTA *</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Label du bouton</label>
            <input className={fieldClass()} value={value.ctaLabel ?? ''} onChange={e => set('ctaLabel', e.target.value)} placeholder="Déposer mon brief" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">URL du bouton</label>
