...
export function TestimonialForm({ testimonial, onClose }: { testimonial: Partial<Testimonial> | null; onClose: (s: boolean) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto bg-[#0B1121] border border-[#1E293B] rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-[#1E293B]">
          <h2 className="text-lg font-bold text-white">{form.id ? 'Modifier le témoignage' : 'Nouveau témoignage'}</h2>
          <button onClick={() => onClose(false)}><X className="w-5 h-5 text-slate-400 hover:text-white" /></button>
        </div>
        <div className="p-6 space-y-5">
          {/* Citation */}
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Citation <span className="text-[#C8A96E]">*</span></label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1.5">Titre / Poste</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1.5">URL logo entreprise</label>
              ...
              <div className="flex items-center gap-2">
                {[1,2,3,4,5].map((n) => (
                  <button key={n} type="button" onClick={() => set('rating', n)}>
                    <Star className={`w-6 h-6 transition-colors ${n <= form.rating ? 'text-[#C8A96E] fill-current' : 'text-slate-600 hover:text-slate-400'}`} />
                  </button>
                ))}
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1.5">Slug service lié</label>
